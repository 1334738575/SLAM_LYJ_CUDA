...
namespace SLAM_LYJ_CUDA
{
	...
	public:
		...
		__device__ float crossaProduct2(const float2& p1, const float2& p2) {
		...
		__device__ char isP2dInTriangleCU(const float2& AB, const float2& BC, const float2& CA,
			...
			float v1 = crossaProduct2(AB, AP);
			float v2 = crossaProduct2(BC, BP);
			float v3 = crossaProduct2(CA, CP);
			if (v1 >= 0 && v2 >= 0 && v3 >= 0)
				return (char)1;
			if (v1 <= 0 && v2 <= 0 && v3 <= 0)
				return (char)1;
