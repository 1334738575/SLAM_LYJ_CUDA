...
namespace SLAM_LYJ_CUDA
{
	...
	__global__ void testDepthAndFidCU(float3* _p3ds, float3* _p2ds, uint3* _faces, float3* _fNormals, unsigned int _fn, int _w, int _h, float* _depths, uint32_t* _fids, CameraCU _cam, BaseCU _base, unsigned int _step) {
		...
		for (unsigned int fi = idx; fi < idx + _step; ++fi) {
			...
			if (maxu < _p2ds[_faces[fi].y].x) maxu = _p2ds[_faces[fi].y].x;
			else if (minu > _p2ds[_faces[fi].y].x) minu = _p2ds[_faces[fi].y].x;
			if (maxv < _p2ds[_faces[fi].y].y) maxv = _p2ds[_faces[fi].y].y;
			else if (minv < _p2ds[_faces[fi].y].y) minv = _p2ds[_faces[fi].y].y;
			if (maxu < _p2ds[_faces[fi].z].x) maxu = _p2ds[_faces[fi].z].x;
			else if (minu > _p2ds[_faces[fi].z].x) minu = _p2ds[_faces[fi].z].x;
			if (maxv < _p2ds[_faces[fi].z].y) maxv = _p2ds[_faces[fi].z].y;
			else if (minv < _p2ds[_faces[fi].z].y) minv = _p2ds[_faces[fi].z].y;
	...
	__device__ void atomicUpdateDepthID(uint64_t* addr, float new_depth, unsigned int new_fid) {
		...
		do {
			...
			if (desired.depth >= expected.depth) break;
